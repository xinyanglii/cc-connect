			...
			case EventResult:
				...
				if fullResponse != "" {
					if e.showContextIndicator && event.InputTokens >= 100 {
						...
						fullResponse += contextIndicator(event.InputTokens, win)
					}
...
func (e *Engine) processInteractiveEvents(state *interactiveState, session *Session, sessions *SessionManager, sessionKey string, msgID string, turnStart time.Time, stopTypingFn func(), sendDone <-chan error, replyCtx any) {
	...
	for {
		...
		case EventResult:
			...
			if !isSilent && e.showContextIndicator {
				if sdkPlausible {
					// Prefer session's real context window over the generic default.
					win := 0
					if reporter, ok := state.agentSession.(ContextUsageReporter); ok {
			...
			if isSilent {
				...
				// Stream preview was active — final text is now in-place. No new message sent.
			...
			// TTS: async voice reply if enabled (skip on silent NO_REPLY turns)
...
channelClosed:
	...
	if len(textParts) > 0 {
		...
		// Silent-reply detection on abnormal-exit branch (mirror normal EventResult path).
		exitIsSilent := isSilentReply(fullResponse)
		if !exitIsSilent {
		...
		if exitIsSilent {
			sp.discard()
			slog.Info("abnormal-exit silent reply suppressed", "session", session.ID)
		} else if toolCount > 0 && segmentStart > 0 {
			sp.discard()
			if segmentStart < len(textParts) {
				unsent := strings.Join(textParts[segmentStart:], "")
...
func (e *Engine) cmdShell(p Platform, msg *Message, raw string) {
	...
	if strings.HasPrefix(shellCmd, "--timeout ") {
		...
		if err != nil || secs <= 0 {
			e.reply(p, msg.ReplyCtx, fmt.Sprintf("Invalid --timeout %q (expect positive integer seconds)", rest[:idx]))
			return
		}
...
func (e *Engine) cmdPs(p Platform, msg *Message, args []string) {
	...
	// Upstream safety check: /ps is only meaningful as a supplement to a turn
	// already in flight. When the session is idle, injecting via
	// agentSession.Send bypasses the session lock and races with concurrent
	// normal messages on the CLI's stdin, so reject instead.
...
// silentReplyRe matches a bare NO_REPLY marker (with optional surrounding whitespace),
...
// stripTrailingSilent strips a trailing NO_REPLY marker. Returns (stripped, true) if
// the marker was found and removed; otherwise (text, false).
...
// prefix of "NO_REPLY". Used to hold streaming output until we know whether the
// agent intends to suppress the response (NO_REPLY marker on its own line).
